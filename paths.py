...
class Create_path:
    ...
    def tj_from_line(self, start_pos, end_pos, time_ttl, t_c, t_actual, sim_time):
        """
        ...
        #v = (end_pos-start_pos)/time_ttl
        #if (t_c >= 0 and t_c < time_ttl/2):
        #    vel = v_max*t_c/(time_ttl/2)
        #    pos = start_pos + t_c*vel/2
        #    acc = [0,0,0]
        
        # accelarting in first 3 second
        if (t_c >= 0 and t_c < time_ttl/2 and t_actual < 3): 
            #vel = v_max*t_c/(time_ttl/2)
        ...
        # added a constraint for not being in the final 5 seconds of flight (simulation)   
        # increasing speed in first half of segment
        elif (t_c >= 0 and t_c < time_ttl/2 and t_actual < sim_time - 5): 
            #vel = v_max*t_c/(time_ttl/2)
        ...
        # minderen van snelheid in laatste helft van segment
        elif (t_c >= time_ttl/2 and t_c <= time_ttl and t_actual < sim_time - 4):
        ...
        # reducing velocity when in final 5 seconds of flight
        else:
            vel = v_max*(time_ttl-t_c)/(time_ttl/2) 
            pos = end_pos - (time_ttl-t_c)*vel/2
            acc = [0,0,0]
    ...
    def rrtstar_path_state(self, t, path, sim_time):  
        ...
        # added a new variable to keep track of the time
        t_actual = t
        ...
        if (t < segm_time):     # if we are not yet at the starting position data[0], go from pos to data[0]
            pos, vel = self.tj_from_line(pos,data[0],segm_time,t, t_actual, sim_time)
        
        elif (t < seg_id*segm_time):  # if time smaller than T (end time) then update position based on where we are now (seg_id)
            pos, vel = self.tj_from_line(data[seg_id-2], data[seg_id-1], segm_time, t - segm_time*(seg_id-1), t_actual, sim_time)

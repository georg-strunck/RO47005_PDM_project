...
class PDcontrolller:
    ...
    def control(self, flat_output, state):
        ...
        speed = np.linalg.norm(np.array(flat_output.get('x_dot')))
        print(speed)
        ...
        # page 29
        rdd_des = np.array(flat_output.get('x_ddot')).reshape(3,1) - np.matmul(self.Kd, error_vel) - np. matmul(self.Kp, error_pos)
